...
As described in REDCapR's [Retrieving Longitudinal and Repeating Structures](https://ouhscbbmc.github.io/REDCapR/articles/longitudinal-and-repeating.html) vignette, the best way to read and write data from projects with longitudinal/repeating elements is to break up the "block matrix" dataset into individual datasets. Each rectangle should have a coherent grain.
...
Please closely read the [Retrieve Protected Token](https://ouhscbbmc.github.io/REDCapR/articles/workflow-read.html#part-2---retrieve-protected-token) section, which has important security implications. The current vignette imports a fake dataset into REDCap, and we'll use a token stored in a local file.
...
To keep this vignette focused on writing/importing/uploading to the server, we'll start with the data that needs to be written. These example tables were prepared by [Raymond Balise](https://github.com/RaymondBalise) for our 2023 [R/Medicine](https://events.linuxfoundation.org/r-medicine/) workshop, "Using REDCap and R to Rapidly Produce Biomedical Publications".
...
As discussed in the [Troubleshooting vignette](https://ouhscbbmc.github.io/REDCapR/articles/TroubleshootingApiCalls.html#writing), we recommend running these two preliminary checks before trying to write the dataset to the server for the very first time.
...
If the row is needs a human to add more values or inspect the existing values consider [marking the instrument](https://ouhscbbmc.github.io/REDCapR/reference/constant.html) "incomplete" or "unverified"; the patient's instrument record will appear red or yellow in REDCap's Record Dashboard. Otherwise consider marking the instrument "complete" so it will appear green.
...
If you encounter problems that can be checked with automation, please tell us in [an issue](https://github.com/OuhscBbmc/REDCapR/issues). We'll work with you to incorporate the new check into `REDCapR::validate_for_write()`.
...
A common transformation is changing strings into the integers that underlie radio buttons. Common approaches are [`dplyr::case_match()`](https://dplyr.tidyverse.org/reference/case_match.html) and using joining to lookup tables (if the mappings are expressed in a csv). Here's an in-line example of `dplyr::case_match()`.
...
2.  `redcap_event_name`: If the project is longitudinal or has arms, this indicates the event. Otherwise, you don't need to add this variable.
3.  `redcap_repeat_instrument`: Indicates the instrument/form that is repeating for these columns.
4.  `redcap_repeat_instance`: Typically a sequential positive integer (*e.g.*, 1, 2, 3, ...) indicating the order.

The combination of these variables needs to be unique. Please read the [Retrieving Longitudinal and Repeating Structures](https://ouhscbbmc.github.io/REDCapR/articles/longitudinal-and-repeating.html) vignette for details of these variables and their meanings.
...
If you're doing basic analyses in R, you've probably used the [readr](https://readr.tidyverse.org/) package to convert the text of a CSV to an R data frame. Under the hood, packages like REDCapR use readr to convert the text from REDCap to produce a data frame.

Three reasonable options when reading from REDCap are:

1.  Let [`readr::read_csv()`](https://readr.tidyverse.org/reference/read_delim.html) make it's best guess.

2.  Keep everything as a string/character, as it lives in `redcap_data`.

3.  Ask [`REDCapR::redcap_metadata_coltypes()`](https://ouhscbbmc.github.io/REDCapR/reference/redcap_metadata_coltypes.html) to inspect project & variable settings to infer the likely type. Then pass this specification to [`REDCapR::redcap_read()`](https://ouhscbbmc.github.io/REDCapR/reference/redcap_read.html)'s `col_types` parameter, which is ultimately used by `readr::read_csv()`.

    ![coltypes](images/coltypes.png)

# Token Security

Several good approaches exist for securely handling passwords. (Two years ago I started a [group to catalog them](https://github.com/vanderbilt-redcap/redcap-token-committee-private), but then I dropped the ball.)
...
2.  Use personal tokens so the audit trail is meaningful.
3.  Adding tokens shouldn't be a hassle. Ideally it's automatic.

## Bad Options

1.  Harded-coded. Fails all three goals. It's appropriate for only demo projects without PHI.
...
2.  Save credentials in an unsecured file

## Good Options

1.  Save credentials in an unsecured file. Then use something like

    ``` r
    credential <- REDCapR::retrieve_credential_local(path_credential, 18L)
    ```

2.  Environmental variables. But they're more difficult for portability & reproducibilty.

3.  [Keyring](https://CRAN.R-project.org/package=keyring) package solutions. Good for security & audits, but doesn't scale well across lots of projects & users.

4.  [Token Server](https://ouhscbbmc.github.io/REDCapR/articles/SecurityDatabase.html). Is my preference, but it takes a few hours to establish. It leverages a separate database's connection with Active Directory (ie, "integrated security"). Users never have to manually save or retrieve tokens as long as they're on the same network as the "token server" database.
...
[REDCap Tools](https://redcap-tools.github.io/projects/) (redcap-tools.github.io/) was created by a people to help inventory the evolving list of packages. Most people using the REDCap API are not REDCap admins, so they don't have access to the Community forums.
